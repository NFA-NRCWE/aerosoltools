aerosoltools.loaders.Partector
==============================

.. py:module:: aerosoltools.loaders.Partector
...
.. py:function:: Load_Partector_file(file, extra_data = False)

   Description:
       Load a Partector LDSA text export and return it as an
       :class:`AerosolAlt` time series with TEM sampling metadata.

   :param file: Path to the Partector ``.txt`` export file.
   :type file: str
   :param extra_data: If ``True``, additional columns (beyond ``LDSA``, ``TEM`` and
                      ``Flow``) are stored in ``extra_data`` indexed by ``Datetime``.
                      Defaults to ``False``.
   :type extra_data: bool, optional

   :returns:     Partector LDSA time series with a datetime index, LDSA, TEM flag,
