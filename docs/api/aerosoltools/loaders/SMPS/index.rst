aerosoltools.loaders.SMPS
=========================

.. py:module:: aerosoltools.loaders.SMPS
...
.. py:function:: Load_SMPS_file(file, extra_data = False)

   Description:
       Load an SMPS size-distribution export and return it as an
       :class:`Aerosol2D` number-size distribution with metadata.

   :param file: Path to the SMPS ``.txt`` or CSV export file.
   :type file: str
   :param extra_data: If ``True``, non-distribution columns (e.g. status, flow, voltages)
                      are stored in ``extra_data`` indexed by ``Datetime``. Defaults to
                      ``False``.
   :type extra_data: bool, optional

   :returns:     SMPS size distributions with a datetime index, total concentration,
   ...
   :raises ValueError: If CSV parsing fails in a way that prevents reading the header or
       main data table.
   :raises Exception: If the weight/unit combination in the SMPS header does not match
