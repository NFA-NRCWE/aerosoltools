aerosoltools.loaders.Common
===========================

.. py:module:: aerosoltools.loaders.Common
...
.. py:function:: file_list(path, search_word = None, max_subfolder = 0, nested_list = False)
   ...
   :type path: str
   :param search_word: Substring that must be present in the **basename** of a file for it
                       to be included in the result. If ``None`` (default), all files are
                       considered.
   :type search_word: str | None, optional
   :param max_subfolder: Maximum depth of subfolders to traverse relative to ``path``:

                         - ``0`` (default): only the root folder.
                         - ``1``: root + its immediate subfolders.
                         - ``2``: root + two levels down, etc.
   :type max_subfolder: int, optional
   :param nested_list: If ``False`` (default), return a flat list of file paths.
                       If ``True``, return a list of lists, where each inner list contains
                       the files found in one directory level.
   :type nested_list: bool, optional

   :returns:     If ``nested_list=False``, a flat list of matching file paths.
                 If ``nested_list=True``, a nested list of file paths grouped by
                 directory.
   :rtype: list[str] | list[list[str]]

   :raises FileNotFoundError: If ``path`` does not exist. Check that the directory path is
...
.. py:function:: Load_data_from_folder(folder_path, load_function, search_word = '', max_subfolder = 0, meta_checklist = None, time_rebin = None, **kwargs)
   ...
   :param folder_path: Path to the folder containing the data files to be loaded.
   :type folder_path: str
   :param load_function: Function that loads a single file and returns an aerosol object,
                         typically an instance of :class:`Aerosol1D`,
                         :class:`Aerosol2D`, or :class:`AerosolAlt`.
   :type load_function: Callable[..., Aerosol1D]
   :param search_word: Substring that must appear in the filename for a file to be
                       considered. If an empty string (default), all files in the
                       directory tree (subject to ``max_subfolder``) are considered.
   :type search_word: str, optional
   :param max_subfolder: Maximum depth of subfolders to traverse relative to ``folder_path``:

                         - ``0`` (default): only the base folder.
                         - ``1``: base + its immediate subfolders.
                         - ``2``: base + two levels down, etc.
   :type max_subfolder: int, optional
   :param meta_checklist: Iterable of metadata keys that must match across all loaded files
                          ...
                          ``("serial_number",)``.
   :type meta_checklist: Sequence[str] | None, optional
   :param time_rebin: Optional resampling rule (e.g. ``"30s"``, ``"1min"``, ``"2H"``,
   ...
   :type time_rebin: str | None, optional
   ...
   :returns:     A combined aerosol object of the same concrete class as the first
                 successfully loaded file. The returned object has:
   :rtype: Aerosol1D | Aerosol2D | AerosolAlt

   :raises FileNotFoundError: If no files matching the given criteria are found in
       ``folder_path``, or if all candidate files fail to load.
       Check the folder path, search pattern, and file permissions.
   :raises TypeError: If ``load_function`` returns an object that is not an instance of
       :class:`Aerosol1D`, :class:`Aerosol2D`, or :class:`AerosolAlt`.
   :raises RuntimeError: If an internal inconsistency occurs during concatenation (e.g.
