aerosoltools.loaders.OPCN3
==========================

.. py:module:: aerosoltools.loaders.OPCN3
...
.. py:function:: Load_OPCN3_file(file, extra_data = False, PM_focus = False)
   ...
   :param file: Path to the OPC-N3 data file.
   :type file: str
   :param extra_data: If ``True``, auxiliary channels (e.g. temperature, RH, flow, PM
                      channels) are stored in ``extra_data`` when supported by the
                      underlying loader. Defaults to ``False``.
   :type extra_data: bool, optional
   :param PM_focus: If ``True``, return a compact PM-focused :class:`AerosolAlt` with
   ...
   :type PM_focus: bool, optional

   :returns:     Parsed OPC-N3 dataset as either a size-resolved number-size
                 distribution or a PM-only time series, depending on ``PM_focus``
                 and the detected file format.
   :rtype: Aerosol2D | AerosolAlt
   ...
   :raises ValueError: If the delimiter detection or CSV parsing fails in a way that
       prevents reading the header or main data table.
   :raises Exception: If the file format cannot be recognised from the first line and
