aerosoltools.aerosolalt
=======================

.. py:module:: aerosoltools.aerosolalt
...
.. py:class:: AerosolAlt(dataframe)
   ...
   :param dataframe: Input data with a time column or time index and one or more scalar
   ...
   .. py:method:: plot_total_conc(ax = None, mark_activities = False, parameter = 0)

      Description:
          Plot a selected scalar channel versus time for an :class:`AerosolAlt`
          object.

      :param ax: Existing Matplotlib axes to draw on. If ``None``, a new figure
                 and axes are created. Defaults to ``None``.
      :type ax: matplotlib.axes.Axes | None, optional
      :param mark_activities: Control highlighting of activity periods defined on the object:

                              * ``False`` – no shading (default).
                              * ``True`` – shade all activities except ``"All data"``.
                              * sequence of str – shade only the named activities.
      :type mark_activities: bool | Sequence[str], optional
      :param parameter: Index or column name of the signal to plot. If ``int``, it is
                        interpreted as a positional index into :attr:`data.columns`. If
                        ``str``, it is treated as a column label. Defaults to ``0``.
      :type parameter: int | str, optional

      :returns:     The figure and axes containing the time-series plot.
      :rtype: tuple[matplotlib.figure.Figure, matplotlib.axes.Axes]

      :raises LookupError: If ``parameter`` does not correspond to a valid column index or
          column label.
   ...
   .. py:method:: summarize(filename = None, *, parameter = 0)

      Description:
          Summarise basic statistics of a selected scalar channel by activity.

      :param filename: Path to an Excel file to write the summary table to. If
                       ``None`` (default), the summary is not saved to disk and is
                       only printed and returned.
      :type filename: str | None, optional
      :param parameter: Index or column name of the signal to summarise. If an
      ...
      :returns:     A summary table with one row per activity (including
